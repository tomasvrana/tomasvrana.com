...
import { isRelative, isPath, languageSetInURL } from '../../helpers/url'
...
const Link = (props) => {
  ...
  const unsupportedProps = [ 'children', 'onClick', 'secondary', 'tracking', 'useLang' ]
  unsupportedProps.forEach(prop => delete linkProps[prop])

  if (isRelativeLink && !isPathLink) {
    return (
      <LanguageConsumer>
        {({ lang }) => (
          <GatsbyLink
