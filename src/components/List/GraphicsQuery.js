  ...
  <LanguageConsumer>
    {({ lang }) => (
      <StaticQuery
        query={graphql`
          query{
            allFile(filter:{relativePath:{glob:"content/pages/graphics/*.md"}}) {
              nodes{
                relativePath,
                childMarkdownRemark{
                  frontmatter {
        ...
        `}
        render={({ allFile }) => {
          const content = allFile.nodes.filter(node => node.relativePath === `content/pages/graphics/${lang}.md`).shift()
