  ...
  <LanguageConsumer>
    {({ lang }) => (
      <StaticQuery
        query={graphql`
          query{
            allFile(filter:{relativePath:{glob:"content/projects/islam/*.md"}}) {
              nodes{
                relativePath,
                childMarkdownRemark{
                  frontmatter {
        ...
        `}
        render={({ allFile }) => {
          const content = allFile.nodes.filter(node => node.relativePath === `content/projects/islam/${lang}.md`).shift()
