  ...
  <LanguageConsumer>
    {({ lang }) => (
      <StaticQuery
        query={graphql`
          query{
            allFile(filter:{relativePath:{glob:"content/projects/matka/*.md"}}) {
              nodes{
                relativePath,
                childMarkdownRemark{
                  frontmatter {
        ...
        `}
        render={({ allFile }) => {
          const content = allFile.nodes.filter(node => node.relativePath === `content/projects/matka/${lang}.md`).shift()
