...
# Gathering info to insert into CircleCI environment variables
DEFAULT_GIT_USERNAME="circleci"
printf "Enter the git username you would like to use for CI deployment commits:\n"
read -p "($DEFAULT_GIT_USERNAME): " GIT_USERNAME
...
GIT_EMAIL=""
printf "\nEnter the git email address you would like to use for CI deployment commits\n** This does not have to be a real email address\n"
while [[ $GIT_EMAIL == "" ]]; do
    read -p ": " GIT_EMAIL
done
...
printf "\n* Enable VCS on CircleCI (allows checks to appear on GitHub PR's)"
